...
def scrape_page(url, conn):
    ...
    for listing in property_listings:
        ...
        cursor = conn.cursor()
        cursor.execute("INSERT INTO properties (title, price, location,sqft) VALUES (?, ?, ?, ?)", (title, price, location, sqft))
        conn.commit()
...
total_pages = 403  # Modify this value based on the total number of pages to scrape
